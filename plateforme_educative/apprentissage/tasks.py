...
def indexer_document_task(self, document_id: str, pdf_path: str):
    ...
    logger.info("ChromaDB : démarrage de l'indexation du document %s", document_id[:8])
    try:
...
def export_course_task(self, export_job_id: str):
    ...
    logger.info("Export : démarrage de l'exportation du cours (Job %s)", export_job_id)
    try:
        ...
        logger.info("Export : exportation terminée (fichier %s)", zip_filename)
...
def import_courses_task(self, import_job_id: str, user_id: str, zip_files: list):
    ...
    logger.info("Import : démarrage de l'importation de %s fichier(s) ZIP", len(zip_files))
                    ...
                    for chap_data in metadata.get('chapitres', []):
                        ...
                        if 'video_fichier' in chap_data:
                            ...
                            if vid_file.exists():
                                ...
                                # Skip signal ONLY if we just successfully restored the HLS
                                if getattr(chapitre, 'is_hls_ready', False):
...
def convertir_video_hls(self, chapitre_id: str):
    ...
    logger.info("HLS : démarrage de l'encodage vidéo (chapitre %s)", chapitre_id)
    try:
        ...
        logger.info("HLS : conversion terminée (playlist %s)", rel_hls_path)
