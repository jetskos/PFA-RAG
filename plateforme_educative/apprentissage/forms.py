...
class ChapitreForm(forms.ModelForm):
    class Meta:
        model = Chapitre
        fields = ('titre', 'description', 'ordre', 'url_video', 'fichier_video', 'actif')
        widgets = {
            'titre': forms.TextInput(attrs={'class': 'form-control'}),
            'description': forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
            'ordre': forms.NumberInput(attrs={'class': 'form-control'}),
            'url_video': forms.URLInput(attrs={'class': 'form-control', 'placeholder': 'https://youtube.com/watch?v=...'}),
            'fichier_video': forms.FileInput(attrs={'class': 'form-control', 'accept': 'video/mp4,video/webm'}),
            'actif': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
        }
...
class DevoirForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Bloquer les dates passées dans le sélecteur
